...
$ native-image -cp "$(clojure -Spath):classes" -H:Name=refl -H:+ReportExceptionStackTraces \
    --features=clj_easy.graal_build_time.InitClojureClasses --no-fallback refl.main
----
We no longer see our reflection warnng and our native image now works just fine:
[source,clojure]
----
$ ./refl
ALL GOOD!
...
==== Specify a Reflection Config [[handcoded-reflection-config]]
...
If we recompile our original reflection example app:
[source,shell]
----
$ mkdir -p classes
$ clojure -M -e "(compile 'refl.main)"
