...
See the https://www.graalvm.org/reference-manual/native-image/Reflection/[GraalVM docs on reflection for details] on the config format.
...
The recommended caller based filter, to not get lots of false positives in the generated config, is:
...
The agent discovers reflection during an example call of your program.
...
It's unclear why the reflection on `java.util.Properties` is made. It is
probably safe to leave it out and probably even recommended as this class will
pull in XML libraries due to its `storeToXML` methods. To exclude this class, you can use an https://www.graalvm.org/reference-manual/native-image/BuildConfiguration/#access-filters[access filter].
