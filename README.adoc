...
=== GraalVM Version
Use the latest https://github.com/graalvm/graalvm-ce-builds/releases[GraalVM Community Edition] or https://www.graalvm.org/downloads/[Oracle GraalVM].
Choose the flavour that makes sense for you.

==== GraalVM Version Scheme Change[[graalvm-version-scheme-change]]
In 2023, the Graal team changed their versioning scheme.
It used to be based on the year and quarter, but it moved to simply match the JDK version.

We used to have, for example, v22.3.2 release (tied to 2022 q3) for each supported JDK.
We now have, for example, GraalVM v25.0.2 (tied to JDK 25).

This version change can make it awkward to refer to past versions of GraalVM.
For the old version scheme, we'll prefix the version with `legacy` like so: `GraalVM legacy v22`.

In 2025, Oracle announced that GraalVM is "detaching from the Java ecosystem chain".
From what we understand, this means that new GraalVM releases will be tied to Java LTS (Long Term Support) releases only.
So we should see future 25.x and then eventually 29.x releases, but not 26.x, 27.x, or 28.x releases.
...
To visualize what is in your native image, you can use the `native-image` command's `--emit build-report` option.
See https://www.graalvm.org/latest/reference-manual/native-image/overview/build-report/[GraalVM native-image build report docs].

TIP: At the time of this writing, build reports are only available in Oracle GraalVM and not in the Community Edition (see <<graalvm-free-flavours>>).

NOTE: You might remember the GraalVM Dashboard.
This tool was retired by the GraalVM team with the release of GraalVM v22.
